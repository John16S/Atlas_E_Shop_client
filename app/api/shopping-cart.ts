...
import { IAddToCartFx, IUpdateCartItemFx } from '@/types/shopping-cart copy'
...
export const addToCartFx = createEffect(
  async ({ url, username, goodId }: IAddToCartFx) => {
    const { data } = await api.post(url, { username, goodId })
